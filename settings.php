...
$rs = db()->prepare('SELECT rooms, COUNT(*) c FROM students WHERE sc_id = ? AND years = ? AND class_id = ? AND deleted_at IS NULL AND stustatus <> 4 GROUP BY rooms ORDER BY rooms');
...
$tot = db()->prepare('SELECT COUNT(*) total, COALESCE(SUM(hit1tested+hit2tested+hit3tested),0) tested FROM students WHERE sc_id = ? AND years = ? AND deleted_at IS NULL AND stustatus <> 4');
