...
$years   = (int)($results[0]['years'] ?? ACADEMIC_YEAR);
...
/* ---- 3) students: คะแนน + ธงสอบแล้ว ตามรอบจริง ---- */
$pdo->prepare("UPDATE students SET `hit{$hittest}` = ?, `hit{$hittest}tested` = 1 WHERE stuid = ?")
    ->execute([$score, $stuid]);

/* ---- 4) studenthit: สรุป denormalize item1..item20 ตามรอบจริง ---- */
...
$params = [
    $stuid, $hittest, $student['stuname'], $student['sc_id'],
